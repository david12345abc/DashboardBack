...
> **KD-M1 / KD-Y1:** данные **реальные** из 1С. Кэш на день. Нет кэша — скрипты 1С запускаются автоматически. Помесячный план ВП (`PLAN_VP_BY_MONTH`) задан **только на 2026 год** (`VP_PLAN_TABLE_YEAR`); для других лет используется единый fallback (как план марта 2026). В `months_calendar` — всегда **12 месяцев 2026**: все планы по сетке, факт только за завершённые месяцы. В ответе `KD-M1` поле **`monthly_data`** — **одна строка** (`last_full_month_row`): план, факт и KPI **последнего полного месяца**; ряд из 12 месяцев — в `months_calendar` (для графика). В дашборде комдира таблица **`KD-T-KPI-SUMMARY`**: до пяти строк по порядку — **KD-M1, KD-M2, KD-M3** (последний полный месяц), **KD-Q1** (последний полный квартал), **KD-Q2** (последний полный месяц; в плитке и графике KD-Q2 по-прежнему квартал).
