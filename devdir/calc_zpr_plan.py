...
Использование:
  python devdir/calc_zpr_plan.py
  python devdir/calc_zpr_plan.py 2026 04
  python devdir/calc_zpr_plan.py 2026-04
...
def count_zpr(session: requests.Session, year: int, month: int) -> int:
    """Количество ЗПР за месяц (план): все с ДатаПриемаВРазработку в периоде."""
...
def main() -> None:
    try:
        ...
        session.auth = AUTH
        print(count_zpr(session, year, month))
