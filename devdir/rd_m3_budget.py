"""KPI RD-M3 (бюджет): план из констант; факт из SQL-бэкапа ``devdir.rd_m3``.

Кэш: ``getkpi/dashboard/devdir_rd_m3_budget_<год>_<месяц>.json`` — раз в день.
"""
...
def _build_rd_m3_budget_monthly_payload(year: int, month: int) -> dict[str, Any]:
    return rd_m3.build_rd_m3_payload(year=year, month=month)
...
def get_rd_m3_budget_ytd(year: int | None = None, month: int | None = None) -> dict | None:
    return get_ytd_via_cache(
        ...
        lock_key_prefix="devdir_rd_m3_budget_sql",
        compute_fn=_build_rd_m3_budget_monthly_payload,
        kpi_id="RD-M3",
    )
