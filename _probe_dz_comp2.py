...
def main() -> None:
    ...
    print("commercial objs", cur.fetchone()[0])
    ...
    for t, obj_cols, dts, nums in tests:
        ...
        for col in obj_cols:
            ...
            print(f"  {col} hit={hit}", flush=True)
            if hit and not obj_col:
                obj_col = col
        if not obj_col:
            continue
        print("  USING", obj_col, flush=True)
        ...
        for ncol in nums:
            ...
            if abs(total) < 1_000_000:
                continue
            print(f"  net {ncol}: {total:,.2f}  dDZ={total - TARGET_DZ:,.2f}", flush=True)
            for dcol in dts:
                cur.execute(
                    f"""
                    SELECT SUM(CASE WHEN s._RecordKind = 1 THEN -s.[{ncol}] ELSE s.[{ncol}] END)
                ...
                print(f"    od {dcol}: {od:,.2f}  dOD={od - TARGET_OD:,.2f}", flush=True)
