"""SH-M5 — обращения, исполненные не в срок (претензии).
...
CACHE_PREFIX = "servhead_sh_m5_claims"
CACHE_SOURCE_TAG = "servhead_sh_m5_claims_payload_v1"
CACHE_VERSION = 1
...
def _build_payload(year: int | None = None, month: int | None = None) -> dict[str, Any]:
    ...
    by_month = fetch_claims_sla_counts_by_month(
        ...
        month_from=1,
        month_to=ref_m,
        log_label="SH-M5/Claims",
        fact_match=is_completed_late,
    )
...
def sh_m5_ytd_cache_path(year: int | None = None, month: int | None = None) -> Path:
    return cache_path_for_period(year, month)


def get_sh_m5_ytd(year: int | None = None, month: int | None = None) -> dict[str, Any] | None:
    ref_y, ref_m = normalize_rd_tile_period(year, month)
    ...
    def _runner() -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.exception("SH-M5: ошибка расчёта обращений не в срок")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="SH-M5: обращения не в срок (факт) / все обращения за месяц (план).",
    )
    parser.add_argument("--year", type=int, default=None)
    parser.add_argument("--month", type=int, default=None)
    args = parser.parse_args()
    print(json.dumps(get_sh_m5_ytd(year=args.year, month=args.month), ensure_ascii=False, indent=2))
