...
CACHE_PREFIX = "servhead_sh_m3_claims"
CACHE_SOURCE_TAG = "servhead_sh_m3_claims_payload_v1"
CACHE_VERSION = 1
...
def sh_m3_ytd_cache_path(year: int | None = None, month: int | None = None) -> Path:
    return cache_path_for_period(year, month)


def get_sh_m3_ytd(year: int | None = None, month: int | None = None) -> dict[str, Any] | None:
    ref_y, ref_m = normalize_rd_tile_period(year, month)
    ...
    def _runner() -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.exception("SH-M3: ошибка расчёта зарегистрированных обращений")
    ...
    return locked_call(f"servhead_sh_m3_{ref_y}_{ref_m:02d}", _runner)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="SH-M3: зарегистрированные обращения (факт) / все обращения за месяц (план).",
    )
    parser.add_argument("--year", type=int, default=None)
    parser.add_argument("--month", type=int, default=None)
    args = parser.parse_args()
    print(json.dumps(get_sh_m3_ytd(year=args.year, month=args.month), ensure_ascii=False, indent=2))
