...
from servhead.claims_common import (
    ...
    build_kpi_period,
    empty_error_payload,
    fetch_claims_counts_by_month,
    kpi_pct,
)

logger = logging.getLogger(__name__)

CACHE_PREFIX = "servhead_sh_m2_claims"
CACHE_SOURCE_TAG = "servhead_sh_m2_claims_payload_v1"
CACHE_VERSION = 1
...
def _build_payload(year: int | None = None, month: int | None = None) -> dict[str, Any]:
    ...
    session.auth = AUTH
    by_month = fetch_claims_counts_by_month(
        session,
        year=ref_y,
        month_from=1,
        month_to=ref_m,
        fact_statuses=STATUS_IN_PROCESS,
        log_label="SH-M2/Claims",
    )
    ...
    return {
        ...
        "debug": {
            "kpi_id": KPI_ID,
            "status": "ok",
            "source": "servhead.sh_m2",
            "odata_entity": CLAIMS_ENTITY,
            "status_in_process": sorted(STATUS_IN_PROCESS),
            "rule": (
                "plan = all claims in month by ДатаРегистрации; "
                "fact = status in Обрабатывается / В обработке"
            ),
...
def sh_m2_ytd_cache_path(year: int | None = None, month: int | None = None) -> Path:
    return cache_path_for_period(year, month)


def get_sh_m2_ytd(year: int | None = None, month: int | None = None) -> dict[str, Any] | None:
    ref_y, ref_m = normalize_rd_tile_period(year, month)
    ...
    def _runner() -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.exception("SH-M2: ошибка расчёта обращений в обработке")
    ...
    return locked_call(f"servhead_sh_m2_{ref_y}_{ref_m:02d}", _runner)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="SH-M2: обращения в обработке (факт) / все обращения за месяц (план).",
    )
    parser.add_argument("--year", type=int, default=None)
    parser.add_argument("--month", type=int, default=None)
    args = parser.parse_args()
    print(json.dumps(get_sh_m2_ytd(year=args.year, month=args.month), ensure_ascii=False, indent=2))
