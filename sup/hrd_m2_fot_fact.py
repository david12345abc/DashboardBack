"""Факт ФОТ службы управления персоналом (HRD-M2) из регистра бухучёта, сч. 26."""
...
SUP_FOT_GROUP = SUP_FOT_SPEC[0][0]
...
def compute_hrd_m2_fot_fact_monthly(year: int, month: int) -> dict[str, Any]:
    """Факт ФОТ СУП за календарный месяц (руб.)."""
    try:
        p_start, p_end = _month_period_bounds(year, month)
        session = requests.Session()
        session.auth = AUTH
        name_to_key, labels = load_fot_structure_map_for_spec(session, SUP_FOT_SPEC)
        totals, article_totals = calc_techdir_spec_reg_fact(session, p_start, p_end, name_to_key)
        row = totals.get(SUP_FOT_GROUP, {})
        ...
        return {
            ...
            "debug": {
                "status": "ok",
                "kpi_id": "HRD-M2-FACT",
                "component": "sup.hrd_m2_fot_fact",
                "cost_articles": list(FOT_SPEC_ARTICLES),
                "department": SUP_FOT_GROUP,
                "structure_labels": labels,
            },
        }
    except Exception as exc:
        logger.exception("HRD-M2: ошибка расчёта факта ФОТ за %d-%02d", year, month)
        return {
            "year": year,
            "month": month,
            "month_name": MONTH_RU.get(month, str(month)),
