"""
IT-M4 — ФОТ отдела информационных технологий в пределах лимита.

Эталон (OData, может содержать ошибки):
  DashboardBack/getkpi/autoit/it_m4_fot.py + it_m4_fot_fact.py
...
Использование:
  python autoit/it_m4.py
  python autoit/it_m4.py --check
  python autoit/it_m4.py 2026
  python autoit/it_m4.py 2026-05
  python autoit/it_m4.py 2026-01 2026-07
"""
...
# Как AUTOIT_FOT_SPEC в it_m4_fot_fact.py
...
AUTOIT_GROUP_ORDER = [t[0] for t in AUTOIT_FOT_SPEC]

# Эталон: кэш DashboardBack (янв–май); июнь в кэше был 0 (ошибка/недогруз) — SQL.
...
def resolve_autoit_department_map(cur) -> tuple[dict[bytes, str], dict[str, str]]:
    ...
    labels: dict[str, str] = {}
    for display, aliases in AUTOIT_FOT_SPEC:
        found = None
...
def calc_it_m4_fot_month(
    year: int,
    month: int,
    sql: SqlConnection | None = None,
) -> dict[str, Any]:
    """Факт ФОТ контура ИТ за календарный месяц + план."""
    ...
    with sql.connect_ctx() as conn:
        ...
        id_to_group, labels = resolve_autoit_department_map(cur)

        dept_ids = list(id_to_group.keys())
        art_ids = list(articles.keys())
        if not dept_ids:
            raise RuntimeError("Не найдены подразделения контура ИТ")
    ...
    groups_out: dict[str, dict[str, float]] = {
        ...
        for name in AUTOIT_GROUP_ORDER
    }
    ...
    return {
        ...
        "debug": {
            "status": "ok",
            "kpi_id": "IT-M4",
            "component": "autoit.it_m4.calc_it_m4_fot_month",
            "sql_tables": {
                "register": TBL_RG,
                "accounts": TBL_ACC,
            ...
            "departments": list(AUTOIT_GROUP_ORDER),
...
def build_monthly_report(
    ...
    for year, month in iter_months(start_period, end_period):
        rows.append(calc_it_m4_fot_month(year, month, sql=sql))
    return rows


def format_report(rows: list[dict[str, Any]]) -> str:
    lines = [
        "ФОТ подразделения в пределах лимита · отдел ИТ (IT-M4 / SQL)",
        f"Источник: {TBL_RG} + {TBL_STRUCT} + {TBL_ARTICLES}, сч. 26, 2 статьи АУП",
        f"Контур: {', '.join(AUTOIT_GROUP_ORDER)}",
        "",
...
def save_report(period_slug: str, rows: list[dict[str, Any]], text: str) -> tuple[Path, Path]:
    txt_path = SCRIPT_DIR / f"it_m4_{period_slug}.txt"
    json_path = SCRIPT_DIR / f"it_m4_{period_slug}.json"
...
def build_it_m4_payload(year: int | None = None, month: int | None = None) -> dict[str, Any]:
    now = date.today()
...
def run_check() -> int:
    print("Сверка ФОТ факт · IT-M4 · 2026 (кэш DashboardBack)")
    all_ok = True
    sql = SqlConnection()
    for month in sorted(REFERENCE_FACT_2026):
        payload = calc_it_m4_fot_month(2026, month, sql=sql)
