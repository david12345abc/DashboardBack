"""
Графики и таблица для ответов API коммерческих подразделений (плитки из commercial_tiles).

График 1 (ежемесячно): все KPI из плиток с периодичностью «ежемесячно».
График 2 (ежеквартально): все KPI из плиток с периодичностью «ежеквартально».
Таблица: по одной строке на каждый ежемесячный KPI — данные за последний полный месяц.

Метаданные контейнеров графиков берутся из kpi_data.json: {prefix}-C1 и {prefix}-C2
(prefix — часть до первого «-» у первого kpi_id в порядке плиток, напр. ОТД из ОТД-M1).
Если C2 нет в JSON — подставляется заглушка.
"""
