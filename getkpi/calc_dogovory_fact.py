...
Логика (воспроизведение 1С-запроса «ВТ_ДоговорыФакт»):
  ...
  6. Дополнительно исключаются заказы клиента с партнёрами из
     EXCLUDED_ORDER_PARTNER_NAMES и с ТД_СопровождениеПродажи = Истина
  7. ЗаказКлиента пуст → OK, иначе НЕ ЗаказКлиента.ТД_НеУчитыватьВПланФакте
  8. СуммаДоговора уже в рублях из регистра, без пересчёта по валюте заказа

Примечание: регистр независимый, ДатаПодписания — ресурс. Корректный
            ``$filter`` с ``datetime'...'`` на этой базе проходит; грузим
            все строки, потому что дальше нужны подписи и вне выбранного месяца.
