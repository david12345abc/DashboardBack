"""Общая загрузка SLA по ``Document_ТД_ЗаявкаВСлужбуСопровождения``."""
...
from getkpi.fot_techdir_fact import AUTH, BASE, EMPTY, load_structure, resolve_department_row
...
def resolve_executor_department_key(
...
def compute_support_sla_monthly(
    year: int,
    month: int,
    *,
    kpi_id: str,
    component: str,
    log_tag: str,
    executor_department_label: str,
    executor_department_aliases: tuple[str, ...],
    plan_rule: str,
) -> dict[str, Any]:
    """План — все заявки за месяц; факт — с подтверждением автора в месяце."""
    counts = {
        "docs_plan": 0,
        "docs_fact": 0,
    ...
    try:
        session = requests.Session()
        session.auth = AUTH
        executor_key = resolve_executor_department_key(
            session,
            department_label=executor_department_label,
            department_aliases=executor_department_aliases,
        )
        if not executor_key:
            return {
                "year": year,
        ...
        logger.exception("%s: ошибка за %d-%02d", log_tag, year, month)
        return {
            "year": year,
            "month": month,
            "plan": None,
