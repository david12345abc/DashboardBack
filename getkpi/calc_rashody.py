...
Источники:
  - Счёт 44  — дебетовый оборот (8 статей затрат; оплата труда и страховые взносы исключены)
...
Алгоритм:
  1. По каждому подразделению суммируется Дт-оборот 44 по 8 статьям.
...
# ── 4 статьи затрат для счёта 44, исключённые из расчёта коммерческой службы ──
EXCLUDED_ART_44_SET = frozenset([
...
# ── Все статьи затрат для счёта 44 до бизнес-исключений ──
ALL_ART_44_SET = frozenset([
...
# ── 8 статей затрат для счёта 44 после исключения оплаты труда и страховых ──
ART_44_SET = ALL_ART_44_SET - EXCLUDED_ART_44_SET
...
def get_rashody_monthly(year: int | None = None,
    ...
    logger.info("calc_rashody: loading data for %d months %s", ref_y, ref_m if force_compute else f"1-{ref_m}")
    t0 = time.time()
