"""
1С-M4 — ФОТ отдела сопровождения 1С в пределах лимита.

Эталон (OData, может содержать ошибки):
  DashboardBack/getkpi/c1auto/c1_m4_fot.py + c1_m4_fot_fact.py
...
Использование:
  python 1cauto/1c_m4.py
  python 1cauto/1c_m4.py --check
  python 1cauto/1c_m4.py 2026
  python 1cauto/1c_m4.py 2026-05
  python 1cauto/1c_m4.py 2026-01 2026-07
"""
...
C1AUTO_GROUP_ORDER = [t[0] for t in C1AUTO_FOT_SPEC]

# Эталон: кэш DashboardBack c1auto_c1_m4_fot_2026_07.json (июль в кэше 0 — недогруз).
...
def resolve_c1auto_department_map(cur) -> tuple[dict[bytes, str], dict[str, str]]:
    ...
    labels: dict[str, str] = {}
    for display, aliases in C1AUTO_FOT_SPEC:
        found = None
...
def calc_c1_m4_fot_month(
    year: int,
    month: int,
    sql: SqlConnection | None = None,
) -> dict[str, Any]:
    """Факт ФОТ контура сопровождения 1С за календарный месяц + план."""
    ...
    with sql.connect_ctx() as conn:
        ...
        id_to_group, labels = resolve_c1auto_department_map(cur)

        dept_ids = list(id_to_group.keys())
        art_ids = list(articles.keys())
        if not dept_ids:
            raise RuntimeError("Не найдены подразделения контура сопровождения 1С")
    ...
    groups_out: dict[str, dict[str, float]] = {
        ...
        for name in C1AUTO_GROUP_ORDER
    }
    ...
    return {
        ...
        "debug": {
            "status": "ok",
            "kpi_id": "1C-M4",
            "component": "1cauto.1c_m4.calc_c1_m4_fot_month",
            "sql_tables": {
                "register": TBL_RG,
                "accounts": TBL_ACC,
            ...
            "departments": list(C1AUTO_GROUP_ORDER),
...
def build_monthly_report(
    ...
    for year, month in iter_months(start_period, end_period):
        rows.append(calc_c1_m4_fot_month(year, month, sql=sql))
    return rows


def format_report(rows: list[dict[str, Any]]) -> str:
    lines = [
        "ФОТ подразделения в пределах лимита · отдел сопровождения 1С (1С-M4 / SQL)",
        f"Источник: {TBL_RG} + {TBL_STRUCT} + {TBL_ARTICLES}, сч. 26, 2 статьи АУП",
        f"Контур: {', '.join(C1AUTO_GROUP_ORDER)}",
        "",
...
def save_report(period_slug: str, rows: list[dict[str, Any]], text: str) -> tuple[Path, Path]:
    txt_path = SCRIPT_DIR / f"1c_m4_{period_slug}.txt"
    json_path = SCRIPT_DIR / f"1c_m4_{period_slug}.json"
...
def build_c1_m4_payload(year: int | None = None, month: int | None = None) -> dict[str, Any]:
    now = date.today()
...
def run_check() -> int:
    print("Сверка ФОТ факт · 1С-M4 · 2026 (кэш DashboardBack, может быть неточен)")
    all_ok = True
    sql = SqlConnection()
    for month in sorted(REFERENCE_FACT_2026):
        payload = calc_c1_m4_fot_month(2026, month, sql=sql)
