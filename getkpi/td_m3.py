...
Эталон экрана 1С: заявки ДС, подразделение в группе «ТЕХНИЧЕСКИЙ ДИРЕКТОР»,
оплачена/закрыта (в SQL — Posted).
...
Логика факта (SQL):
  ...
  где:
    ...
    • Подразделение ∈ поддерево корня «ТЕХНИЧЕСКИЙ ДИРЕКТОР»
      (сам корень + все потомки в Catalog_СтруктураПредприятия)
...
# Эталон факта: live SQL по полному поддереву «ТЕХНИЧЕСКИЙ ДИРЕКТОР».
