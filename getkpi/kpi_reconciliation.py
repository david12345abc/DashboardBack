...
ODP_RECONCILIATION_QUESTIONS_RU: list[str] = [
    ...
    (
        "Колонка факта отгрузок: совпадает ли она с «Итого выручка» отчёта "
        "«Валовая прибыль предприятия» (блок «По подразделениям»), а не с "
        "AccumulationRegister.РаспоряженияНаОтгрузку?"
    ),
]
