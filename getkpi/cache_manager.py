...
def _build_warm_tasks(ref_y: int, ref_m: int) -> list[tuple[str, Path, object]]:
    ...
    from sup import hrd_m1, hrd_m2, hrd_m3, hrd_m4, hrd_m5, hrd_m6, hrd_m7, hrd_m9, hrd_q4
    ...
    _append_sup_warm_tasks(
        tasks, y, m,
        hrd_m1, hrd_m2, hrd_m3, hrd_m4, hrd_m5, hrd_m6, hrd_m7, hrd_m9, hrd_q4,
    )
...
def _append_sup_warm_tasks(
    ...
    """Прогреть файловые кэши SUP (HRD-M1…M7, M9, HRD-Q4) за все месяцы 1..ref_m."""
    for warm_m in range(1, ref_m + 1):
        tasks.append((
        ...
        tasks.append((
            f"sup_hrd_m5_{ref_y}_{warm_m:02d}",
            hrd_m5_mod.cache_file_path_for_period(ref_y, warm_m),
            lambda yy=ref_y, mm=warm_m: hrd_m5_mod.get_hrd_m5_ytd(year=yy, month=mm),
        ))
        tasks.append((
            f"sup_hrd_m6_{ref_y}_{warm_m:02d}",
            hrd_m6_mod.cache_file_path_for_period(ref_y, warm_m),
            lambda yy=ref_y, mm=warm_m: hrd_m6_mod.get_hrd_m6_ytd(year=yy, month=mm),
        ))
        tasks.append((
