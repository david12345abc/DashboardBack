"""
probe_klient_register.py — диагностика регистра РасчетыСКлиентамиПоСрокам.

Показывает:
  1) все поля сущности регистра (через $metadata);
  2) 2 примера записей с полным содержимым (все поля, как их отдаёт 1С);
  3) список уникальных значений RecordType;
  4) сумму всех числовых ресурсов с разбивкой по RecordType — чтобы увидеть,
     есть ли в регистре отрицательные/парные записи для «нашего долга».

Запуск на сервере:
  cd /path/to/Back/Dashbord
  python probe_klient_register.py
"""
...
ENTITY_META = "AccumulationRegister_РасчетыСКлиентамиПоСрокам"
...
def extract_entity_schema(xml: str, entity_name: str) -> list[str]:
    """
    Наивный парсер: из $metadata вытягивает список свойств для EntityType с
    указанным именем.
    """
...
def fetch_stats(session: requests.Session, date_str: str) -> tuple[dict, dict]:
    """
    Тянет ВСЕ записи регистра за Period <= date_str, считает:
      - уникальные RecordType;
      - суммы ДолгУпр / ДолгРегл / Предоплата* и т.п. по RecordType.
    """
