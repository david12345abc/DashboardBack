...
def _tile_month_pairs(year: int, ref_month: int) -> list[tuple[int, int]]:
    if year == 2026 and QD_M3_PLAN_BY_MONTH_2026:
        upper_month = max(max(QD_M3_PLAN_BY_MONTH_2026), ref_month)
    else:
...
def get_qd_m3_ytd(year: int | None = None, month: int | None = None) -> dict[str, Any]:
    ...
    return locked_call(f"qualdir_qd_m3_{lock_y}_{lock_m:02d}", _runner)
