"""
QD-M6 — предъявления продукции на входной контроль (qualdir).

Источник: ``Document_ТД_ПредъявлениеТМЦнаОТК``.

На плитке за выбранный месяц:
  1) ``fact`` — число документов по реквизиту ``Date``;
  2) ``in_work_today`` — документы со ``СрокИсполнения`` = сегодня;
  3) ``delay_count`` — ``СрокИсполнения`` в месяце и < ``ДатаПроверкиОТК``.

Кэш: помесячно ``qualdir_otk_predyavlenie_<Y>_<MM>.json``; YTD —
``qualdir_qd_m6_ytd_<Y>_<MM>.json``.
"""
...
from qualdir.otk_predyavlenie_data import (
    DOC_ENTITY,
    compute_in_work_today,
    compute_otk_predyavlenie_month,
)

logger = logging.getLogger(__name__)

_CACHE_ROOT = Path(__file__).resolve().parent.parent / "getkpi" / "dashboard"
SOURCE_TAG = "qualdir_otk_predyavlenie_month_v1"
CACHE_VERSION = 1

QD_M6_YTD_CACHE_PREFIX = "qualdir_qd_m6_ytd"
QD_M6_YTD_DISK_TAG = "qualdir_qd_m6_ytd_payload_v1"
QD_M6_YTD_DISK_VERSION = 1
...
def otk_predyavlenie_month_cache_path(year: int, month: int) -> Path:
    _CACHE_ROOT.mkdir(parents=True, exist_ok=True)
    return _CACHE_ROOT / f"qualdir_otk_predyavlenie_{year}_{month:02d}.json"


def qd_m6_ytd_cache_path(year: int | None = None, month: int | None = None) -> Path:
    ref_y, ref_m = _normalize_period(year, month)
    return ytd_json_cache.cache_path(QD_M6_YTD_CACHE_PREFIX, ref_y, ref_m)


def qd_m6_tile_cache_path(year: int, month: int) -> Path:
...
def _load_month_cache(year: int, month: int) -> dict[str, Any] | None:
    path = otk_predyavlenie_month_cache_path(year, month)
    if not path.exists():
        return None
    try:
...
def _save_month_cache(year: int, month: int, payload: dict[str, Any]) -> None:
    try:
        with otk_predyavlenie_month_cache_path(year, month).open("w", encoding="utf-8") as handle:
            json.dump(
                {
                    **payload,
                    "source": SOURCE_TAG,
...
def compute_qd_m6_month(
    ...
    *,
    ...
    snapshot = compute_otk_predyavlenie_month(year, month, session=session)
...
def _month_row_from_snapshot(snapshot: dict[str, Any], y: int, m: int) -> dict[str, Any]:
    ...
    delay_count = snapshot.get("delay_count")
    ...
    row: dict[str, Any] = {
        ...
        "delay_count": int(delay_count) if has_data and delay_count is not None else None,
        "kpi_pct": None,
        "has_data": has_data,
    }
    if has_data:
...
def _compute_qd_m6_tile(ref_y: int, ref_m: int) -> dict[str, Any]:
    ...
    try:
        for y, m in _month_pairs(ref_y, ref_m):
            try:
                snapshot = compute_qd_m6_month(y, m, session=session)
            except requests.RequestException as exc:
                logger.warning("QD-M6: OData error %d-%02d: %s", y, m, exc)
                snapshot = {
                    "year": y,
                    "month": m,
                    "docs_count": None,
        ...
        in_work = compute_in_work_today(session=session)
        ...
        return {
            ...
            "ytd": {
                ...
                "delay_count": ref_row.get("delay_count") if ref_row else None,
...
def get_qd_m6_ytd(year: int | None = None, month: int | None = None) -> dict[str, Any]:
    ref_y, ref_m = _normalize_period(year, month)
    disk_path = qd_m6_ytd_cache_path(ref_y, ref_m)
    ...
    def _runner() -> dict[str, Any]:
        ...
        try:
            payload = _compute_qd_m6_tile(ref_y, ref_m)
        except Exception as exc:
            logger.exception("Ошибка при расчёте QD-M6 (предъявления на ОТК)")
            payload = {
                "data_granularity": "monthly",
                "monthly_data": [],
    ...
    return locked_call(f"qualdir_qd_m6_{ref_y}_{ref_m:02d}", _runner)
